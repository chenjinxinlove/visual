...
function mapPlay(){
  ...
  map_interval=setInterval(function(){showDate()},150)
    ...
    d3.json("http://datanews.caixin.com/mobile/flood/en/js/data/rain.json", function(error, root) {
...
//==颜色取值==//
var index = [0, 1, 2, 3];
/*var colorPool = ["rgba(34,124,186,0.2)", 
                 "rgba(34,124,186,0.2)", 
                 "rgba(34,124,186,0.2)",
                 "rgba(34,124,186,0.2)", 
                 "rgba(34,124,186,0.2)", 
...
//日期坐标
var monthCaption=[["MAR",31],["APR",30],["MAY",31],["JUNE",30],["JULY",31],["AUG",31],["SEP",30],["OCT",16]];
var sumdays=0;
...
/*
d3.json("http://datanews.caixin.com/mobile/flood/en/js/data/china.json", function(error, root) {
...
function drawCityButton(){
  ...
  d3.json("http://datanews.caixin.com/mobile/flood/en/js/data/cities.json", function(error, root) {    
    if (error) 
      return console.error(error);
...
function clickCityButton(city){
    ...
    d3.json("http://datanews.caixin.com/mobile/flood/en/js/data/rain_full.json", function(error, root) {
      ...
      d3.select(".map-part1-cityline").on("mousemove",function(d){
        ...
        if(count>=0&&count<=229){
          ...
          $(".map-line-number").html("precipitation:<br/>"+c[count]+"mm");
      ...
      $(".map-line-number").html("precipitation:<br/>"+c[initDate]+"mm");
