...
gulp.task('open-ww', function (done) {
  gulp.src('')
    .pipe(gulpOpen({
      app: browser,
      uri: 'http://tq121.weather.com.cn/ski/' + pathN + '/'
    }))
    .on('end', done);
});
...
//外网
gulp.task('ftp_ww', runSequence('clean','imgmin',['sassmin', 'build-js','fonts', 'json'], 'md5:css', 'md5:js','fileinclude','ftp-ww', 'open-ww'));
//微信
gulp.task('ftp_wx', runSequence('clean','imgmin',['sassmin', 'build-js','fonts', 'json'], 'md5:css', 'md5:js','fileinclude','ftp-wx', 'open-wx'));
//发布
